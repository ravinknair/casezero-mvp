export default async function LoginPage({ searchParams }: { searchParams: Promise<{ invitation?: string }> }) {
	const { invitation } = await searchParams;
	const signInUrl = invitation ? `/api/auth/github?invitation=${encodeURIComponent(invitation)}` : "/api/auth/github";
	return (
		<main className="flex min-h-screen items-center justify-center bg-gray-50 p-6">
			<section className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-8 shadow-sm">
				...
				<a className="mt-6 block rounded bg-gray-900 px-4 py-3 text-center text-sm font-semibold text-white hover:bg-gray-700" href={signInUrl}>Continue with GitHub</a>
			</section>
		</main>
	);
}